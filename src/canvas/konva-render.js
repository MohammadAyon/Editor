...
function eventClientPoint(nativeEvent){
  const touch = nativeEvent && (
    (nativeEvent.touches && nativeEvent.touches[0]) ||
    (nativeEvent.changedTouches && nativeEvent.changedTouches[0])
  );
  return touch || nativeEvent;
}

function preventTouchScroll(nativeEvent){
  if(nativeEvent && String(nativeEvent.type || '').startsWith('touch') && nativeEvent.cancelable){
...
export function getStagePointer(nativeEvent){
  ...
  const point = eventClientPoint(nativeEvent);
  if(!point) return null;
  return {
    x: (point.clientX - rect.left) * konvaStage.width() / rect.width,
    y: (point.clientY - rect.top) * konvaStage.height() / rect.height
  };
}
...
export function makeKonvaNode(el){
  ...
  dragStartNode.on('mousedown touchstart', event => {
    const native = event.evt;
    native.stopPropagation();
    preventTouchScroll(native);
